# AIAction Unified Architecture Proposal

## Overview

This document proposes a unified `AIAction` interface that serves as the common foundation for both MCP tools and A2A skills, enabling code reuse and consistent behavior across AI protocols in openHAB.

## Table of Contents

1. [Current State Analysis](#current-state-analysis)
2. [Commonalities and Differences](#commonalities-and-differences)
3. [AIAction Interface Design](#aiaction-interface-design)
...
## AIAction Interface Design

### Core AIAction Interface
...
/**
 * Adapter that converts AIAction to MCPTool interface.
 */
public class MCPToolAdapter implements MCPTool {
    ...
    public MCPToolValidationResult validateParameters(Map<String, Object> parameters) {
        AIActionValidationResult validation = action.validateParameters(parameters);
    ...
    private AIActionContext convertToAIActionContext(MCPToolContext mcpContext) {
        return AIActionContext.builder()
            .protocol("mcp")
    ...
    private MCPToolResult convertToMCPToolResult(AIActionResult aiResult) {
...
/**
 * Adapter that converts AIAction to A2A skill interface.
 */
public class A2ASkillAdapter implements A2ASkill {
    ...
    public A2ASkillAdapter(AIAction action) {
        this.action = action;
    ...
    public CompletableFuture<A2ASkillResult> executeAsync(A2AMessage message) {
        try {
            ...
            // Convert A2A context to AIAction context
            AIActionContext aiContext = convertToAIActionContext(message);
    ...
    private AIActionContext convertToAIActionContext(A2AMessage message) {
        return AIActionContext.builder()
            .protocol("a2a")
    ...
    private A2ASkillResult convertToA2ASkillResult(AIActionResult aiResult) {
        if (aiResult.isSuccess()) {
...
### Phase 1: Core AIAction Interface

1. **Create AIAction Interface**
   ```java
...
2. **Implement Supporting Classes**
   ```java
   // AIActionContext, AIActionResult, AIActionValidationResult, AIActionMetadata
   ```
...
1. **Migrate Existing Tools**
   ```java
   // Convert existing MCP tools to AIAction implementations
   public class ListItemsAction extends AbstractAIAction {
       // Implementation using AIAction interface
   }
   ```
...
2. **Create New Actions**
   ...
   public class SceneCoordinationAction extends AbstractAIAction {
...
2. **Protocol-Specific Registries**
   ```java
   // MCP Tool Registry using AIAction Registry
   public class MCPToolRegistry {
       private final AIActionRegistry actionRegistry;
       
       public MCPTool getTool(String toolId) {
           AIAction action = actionRegistry.getAction(extractActionId(toolId));
...
1. **🔗 Interface Complexity**
   - AIAction interface must support both protocols
...
3. **📚 Learning Curve**
   - Developers need to understand AIAction interface
...
import org.openhab.core.ai.common.api.action.AbstractAIAction;
...
public class ListItemsAction extends AbstractAIAction {
    ...
    public AIActionResult execute(Map<String, Object> parameters, AIActionContext context) {
        ...
        try {
            ...
            return AIActionResult.success(items, executionTime);
            
        } catch (Exception e) {
            long executionTime = System.currentTimeMillis() - startTime;
            return AIActionResult.error("Failed to list items: " + e.getMessage(), 
                new AIActionError("EXECUTION_ERROR", e.getMessage()), executionTime);
        }
    }
...
The `AIAction` unified architecture provides a powerful foundation for sharing AI capabilities between MCP and A2A protocols while maintaining protocol-specific features and optimizations. The adapter pattern ensures clean separation of concerns while enabling maximum code reuse.
...
1. Start with core AIAction interface
...
- **Purpose**: Propose unified AIAction architecture for protocol interoperability
