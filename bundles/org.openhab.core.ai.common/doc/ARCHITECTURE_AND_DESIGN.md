...
4. [Unified AIAction Interface](#unified-aiaction-interface)
...
- **Code Reuse**: Shared AIAction implementations across protocols
...
1. **Single Source of Truth**: AIActions implemented once in the common bundle
...
- **OpenHAB as Single Agent**: One intelligent agent with 68+ skills (AIActions)
...
├── org.openhab.core.ai.common     # Shared foundation (68+ AIActions)
...
- **Responsibilities**:
  ...
  - AIAction implementations (68+ actions)
...
## Unified AIAction Interface

### Design Philosophy

The unified `AIAction` interface serves as the common foundation for both MCP tools and A2A skills, enabling maximum code reuse while maintaining protocol-specific features.
...
public interface AIAction {
    ...
    AIActionMetadata getMetadata();              // Comprehensive metadata
    Map<String, Object> getCapabilities();      // Action capabilities
    void initialize(AIActionContext context);    // Initialization
...
public class MCPToolAdapter implements MCPTool {
    ...
    public MCPToolResult execute(Map<String, Object> parameters, MCPToolContext context) throws MCPToolException {
        ...
        AIActionResult result = aiAction.execute(parameters, aiContext);
...
public class A2ASkillAdapter implements A2ASkill {
    ...
    public CompletableFuture<A2ASkillResult> executeAsync(A2AMessage message) {
        ...
        return aiAction.executeAsync(parameters, aiContext)
            .thenApply(this::convertToA2ASkillResult);
    }
}
```

### AIAction Categories and Distribution
...
```
MCP Client → MCP Server → MCPToolAdapter → AIAction → OpenHAB Services
                ↓
           Tool Registry ← AIActionRegistry
```

**Key Integration Points:**
- **SDK Integration**: Uses official MCP Java SDK classes
- **Transport Support**: STDIO, SSE, and WebSocket transports
- **Tool Discovery**: Dynamic tool registration from AIActions
- **Authentication**: Integrated with common security framework

### A2A Integration Architecture

```
External Agent → A2A Server → A2ASkillAdapter → AIAction → OpenHAB Services
                     ↓
                Skill Registry ← AIActionRegistry
                     ↓
...
### AIAction Relevance Analysis

Based on comprehensive analysis of 68+ AIActions, the distribution for external agent use:
...
Beyond the shared AIActions, A2A requires additional skills for cross-system coordination:
...
1. **Unified Foundation**: Single AIAction interface for all protocols
...
1. **Phase 1**: Complete AIAction interface implementation
