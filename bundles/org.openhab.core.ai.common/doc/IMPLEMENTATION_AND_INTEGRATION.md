...
AIActionContext context = AIActionContext.builder()
    .protocol("mcp")
    .clientId(clientId)
...
1. **Agent-to-Agent Communication**
   ...
   - `AgentSkill` mapping from AIActions for skill exposition
...
public class A2AOpenHABPersistenceManager {
...
2. **Action Execution**: Real AI actions executed through `AIActionRegistry`
...
```java
@Component(service = AIAction.class)
public class GetThingAction extends AbstractAIAction {
    ...
    public AIActionResult execute(Map<String, Object> parameters, AIActionContext context) {
        ...
        if (thing == null) {
            return AIActionResult.error("Thing not found: " + thingUID);
        }
        
        return AIActionResult.success(convertThingToMap(thing), executionTime);
    }
}
```
