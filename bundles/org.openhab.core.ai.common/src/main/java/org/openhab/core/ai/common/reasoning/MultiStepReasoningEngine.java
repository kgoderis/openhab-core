...
import org.openhab.core.ai.common.api.reasoning.ReasoningStep;
...
public class MultiStepReasoningEngine {
    ...
    @Reference
    private @Nullable LLMClient llmClient;
    ...
    private MultiStepReasoningResult executeReasoningSession(String sessionId, ReasoningContext context,
        ...
        List<AIActionResult> actions = new ArrayList<>();
        ...
        for (int stepNumber = 1; stepNumber <= configuration.getMaxSteps(); stepNumber++) {
            ...
            try {
                ...
                if (step.getToolCalls() != null && !step.getToolCalls().isEmpty()) {
                    for (AIActionContext action : step.getToolCalls()) {
                        AIActionResult executedAction = executeAction(action, context);
    ...
    private ReasoningStep executeReasoningStep(String sessionId, int stepNumber, String accumulatedContext,
        ...
        LLMResponse response = executeLLMReasoning(prompt, context);

        // Parse reasoning and actions
        String reasoning = extractReasoning(response);
        List<AIActionContext> actions = parseResponseActions(response);
     ...
     * Execute LLM reasoning
     */
    private LLMResponse executeLLMReasoning(String prompt, ReasoningContext context) {
        LLMClient client = llmClient;
        if (client == null) {
            throw new IllegalStateException("LLM client not available");
        }

        LLMParameters parameters = LLMParameters.builder().maxTokens(configuration.getMaxTokensPerStep())
     ...
     */
    private String extractReasoning(LLMResponse response) {
     ...
     */
    private List<AIActionContext> parseResponseActions(LLMResponse response) {
     ...
     * Execute an action
     */
    private AIActionResult executeAction(AIActionContext action, ReasoningContext context) {
        Instant startTime = Instant.now();

        try {
            // Execute the action
            AIActionResult result = executeActionInternal(action);
            return result;

        } catch (Exception e) {
            logger.error("Action {} failed", action.getCorrelationId(), e);
            String errorMessage = e.getMessage() != null ? e.getMessage() : "Unknown error";
            return AIActionResult.error("Action execution failed: " + errorMessage,
                    new AIActionError("EXECUTION_ERROR", errorMessage),
                    Duration.between(startTime, Instant.now()).toMillis());
        }
    }

    /**
     * Execute an action using AIActionRegistry
     */
    private AIActionResult executeActionInternal(AIActionContext action) {
        AIActionRegistry registry = actionRegistry;
        if (registry == null) {
            throw new IllegalStateException("AIActionRegistry not available");
        }
     ...
     */
    private boolean isReasoningComplete(String reasoning, List<AIActionContext> actions) {
     ...
     */
    private double calculateStepConfidence(String reasoning, List<AIActionContext> actions) {
        ...
        // For now, we don't have success/failure info in AIActionContext
     ...
     */
    private String accumulateActionResult(String currentContext, AIActionResult action) {
