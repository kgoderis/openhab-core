 ...
 */
@NonNullByDefault
public class LocalAIClientImpl implements LLMClient {

    private final Logger logger = LoggerFactory.getLogger(LocalAIClientImpl.class);
    ...
    public LocalAIClientImpl(LocalAIConfiguration config, @Nullable AIActionRegistry actionRegistry) {
        this.config = config;
        ...
        this.providerInfo = new LLMProviderInfo(LLMProviderType.LOCALAI, config.getModelName(), true, // supportsFunctionCalling
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                return LLMResponse.builder().content(responseContent).modelName(config.getModelName())
                        .providerType(LLMProviderType.LOCALAI.name()).build();
    ...
    public LLMHealthStatus getHealthStatus() {
        try {
            ...
            return new LLMHealthStatus(available, now, available ? 100L : 0L, available ? 1.0 : 0.0, 0, null, null);
        } catch (Exception e) {
            Instant now = Instant.now();
            return new LLMHealthStatus(false, now, 0L, 0.0, 1, e.getMessage(), now);
        }
    }

    @Override
    public LLMProviderType getProviderType() {
        return LLMProviderType.LOCALAI;
    }
    ...
    public boolean supportsFunctionCalling() {
        // TODO: Check if LocalAI supports function calling
        return false;
    }
    ...
    public boolean supportsMultimodal() {
        // TODO: Check if LocalAI supports multimodal input
        return false;
    }
