...
/**
 * Statistics class for agent task persistence analysis.
 * 
 * <p>
 * This class provides computed statistics and insights about agent task persistence
 * operations derived from metrics data over time ranges. It implements capability interfaces
 ...
 */
@NonNullByDefault
public record AgentTaskPersistenceStatistics(List<AgentTaskSnapshot> snapshots, Duration timeRange, long timestampMs)
        implements
            StatisticsSnapshot,
            CountsMetrics,
            LatencyMetrics,
    ...
    public double dataIntegrityRate() {
        if (snapshots.isEmpty())
            return 0.0;
        // Calculate data integrity based on success rate and decision accuracy
        return snapshots.stream().mapToDouble(s -> (s.successRate() + s.decisionAccuracy()) / 2.0).average()
                .orElse(0.0);
    }
    ...
    public double storageEfficiency() {
        if (snapshots.isEmpty())
            return 0.0;
        // Calculate storage efficiency based on success rate
        return snapshots.stream().mapToDouble(s -> s.successRate()).average().orElse(0.0);
    }
    ...
    public double recoverySuccessRate() {
        if (snapshots.isEmpty())
            return 0.0;
        // Calculate recovery success rate based on success rate
        return snapshots.stream().mapToDouble(s -> s.successRate()).average().orElse(0.0);
    }
    ...
    public double backupFrequency() {
        if (timeRange.toHours() == 0)
            return 0.0;
        return total() / timeRange.toHours();
    }
    ...
    public double dataRetentionEfficiency() {
        if (snapshots.isEmpty())
            return 0.0;
        // Calculate data retention efficiency based on success rate
        return snapshots.stream().mapToDouble(s -> s.successRate()).average().orElse(0.0);
    }
     ...
     * @return agent task persistence statistics
     */
    public static AgentTaskPersistenceStatistics fromSnapshots(List<AgentTaskSnapshot> snapshots, Duration timeRange) {
        return new AgentTaskPersistenceStatistics(snapshots, timeRange, System.currentTimeMillis());
    }
     ...
     * 
     * @param timeRange the time range for statistics
     * @return empty agent task persistence statistics
     */
    public static AgentTaskPersistenceStatistics empty(Duration timeRange) {
        return new AgentTaskPersistenceStatistics(List.of(), timeRange, System.currentTimeMillis());
    }
}
