...
public final class BehaviorPatternEntry {
    ...
    public BehaviorPatternEntry(String interactionType) { this.interactionType = interactionType; }

    public boolean addInteraction(Map<String, Object> interactionData) {
        recentInteractions.add(interactionData);
        if (recentInteractions.size() > 100) { recentInteractions.remove(0); }
    ...
    private boolean detectPattern() {
        if (recentInteractions.size() < 3) { return false; }
        return recentInteractions.size() >= 5;
    }

    private void updateConfidence() { confidence = Math.min(1.0, recentInteractions.size() / 10.0); }
