...
public interface FullMetrics
        extends CountsMetrics, LatencyMetrics, ModelMetrics, HealthMetrics, ErrorMetrics, ThroughputMetrics {
    ...
    default String getHealthSummary() {
        ...
        summary.append("Health: ").append(healthStatus()).append(" (Score: ")
                .append(String.format("%.2f", healthScore())).append(")");
    ...
    default boolean isPerformingWell() {
        return isHealthy() && successRate() >= 0.95 && errorRatePercentage(total()) <= 5.0
    ...
    default boolean needsAttention() {
        return !isHealthy() || successRate() < 0.9 || errorRatePercentage(total()) > 10.0
