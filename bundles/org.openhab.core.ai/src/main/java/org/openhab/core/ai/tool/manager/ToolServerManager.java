...
import org.openhab.core.ai.tool.ToolServer;
import org.openhab.core.ai.tool.ToolServerConfiguration;
...
public class ToolServerManager implements ReadyTracker {

    private static final Logger logger = LoggerFactory.getLogger(ToolServerManager.class);
    ...
    private final Map<String, ToolServer> serverInstances = new ConcurrentHashMap<>();
     ...
     */
    public ToolServer getServerInstance(String serverId) {
        ...
        ToolServer server = serverInstances.get(serverId);
     ...
     */
    public ToolServer createServerInstance(String serverId, ToolServerConfiguration config) throws Exception {
        ...
        try {
            ...
            ToolServer server = new ToolServer(serverId, config, registry);
    ...
    public boolean removeServerInstance(String serverId) {
        ...
        ToolServer server = serverInstances.remove(serverId);
    ...
    private ToolServerConfiguration loadConfigurationFromService() {
        ...
        try {
            ToolServerConfiguration.Builder builder = ToolServerConfiguration.builder();
            ...
            builder.transportType(ToolTransportType.valueOf(transportType))
            ...
            ToolServerConfiguration config = builder.build();
    ...
    private ToolServerConfiguration createDefaultConfiguration() {
        return ToolServerConfiguration.builder().serverId("openhab-tool-server").serverName("openHAB Tool Server")
                .serverVersion("1.0.0").transportType(ToolTransportType.STDIO).enableTools(true).enableResources(true)
