 ...
 * @since 1.0.0
 */
@Component(service = AgentExecutorImpl.class)
@NonNullByDefault
public class AgentExecutorImpl implements AgentExecutor {

    private final Logger logger = LoggerFactory.getLogger(AgentExecutorImpl.class);
    ...
    @Reference
    public void setSynchronizationService(AgentSynchronizationService synchronizationService) {
        this.synchronizationService = synchronizationService;
    }

    @Override
    public void execute(@NonNullByDefault RequestContext requestContext, @NonNullByDefault EventQueue eventQueue)
            throws JSONRPCError {
    ...
    @Override
    public void cancel(@NonNullByDefault RequestContext requestContext, @NonNullByDefault EventQueue eventQueue)
            throws JSONRPCError {
        // Implementation without @NonNullByDefault to avoid parameter redefinition
        try {
            String taskId = requestContext.getTaskId();
    ...
    private ActionContext createActionContext(Task task, AuthenticationContext authContext) {
        // Use the builder pattern for ActionContext
        ActionContext.Builder builder = ActionContext.builder().protocol("a2a").clientId(extractClientIdFromTask(task))
     ...
     */
    @NonNullByDefault
    public CompletableFuture<AgentSynchronizationService.TransactionResult> executeTaskWithTransaction(Task task,
            EventQueue eventQueue) {
        if (synchronizationService == null) {
            CompletableFuture<AgentSynchronizationService.TransactionResult> future = new CompletableFuture<>();
     ...
     */
    @NonNullByDefault
    public AgentSynchronizationService.TaskExecutionStats getSynchronizationStats() {
        if (synchronizationService == null) {
            return new AgentSynchronizationService.TaskExecutionStats(0, Map.of(), 0, 0);
        }
