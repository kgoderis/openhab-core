...
public record NegotiationResult(
		...
		String message,
		@Nullable Map<String, Object> agreement,
		@Nullable NegotiationProposal proposal) {

	public static NegotiationResult agreement(NegotiationSession session, Map<String, Object> agreement) {
		return new NegotiationResult(session.getSessionId(), NegotiationStatus.AGREED, "Agreement reached", agreement, null);
	}

	public static NegotiationResult proposalAccepted(NegotiationSession session, NegotiationProposal proposal) {
		return new NegotiationResult(session.getSessionId(), NegotiationStatus.ACTIVE, "Proposal accepted", null, proposal);
	}

	public static NegotiationResult aborted(NegotiationSession session, String reason) {
		return new NegotiationResult(session.getSessionId(), NegotiationStatus.ABORTED, "Aborted: " + reason, null, null);
	}
