 ...
 */
@NonNullByDefault
public class VModelClientImpl implements ModelClient {

    private final Logger logger = LoggerFactory.getLogger(VModelClientImpl.class);
    ...
    public VModelClientImpl(VLLMConfiguration config, @Nullable ActionRegistry actionRegistry) {
        this.config = config;
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                return ModelResponse.builder().content(responseContent).modelName(config.getModelName())
                        .providerType(ModelProviderType.VLLM.name()).build();
        ...
        return CompletableFuture.supplyAsync(() -> {
            try {
                ...
                ModelResponse llmResponse = ModelResponse.builder().content(finalContent)
                        .modelName(config.getModelName()).providerType(ModelProviderType.VLLM.name()).build();
