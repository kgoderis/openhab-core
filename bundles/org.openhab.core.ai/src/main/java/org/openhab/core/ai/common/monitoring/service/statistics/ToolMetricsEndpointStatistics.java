...
import org.openhab.core.ai.common.monitoring.api.EndpointMetrics;
...
/**
 * Statistics class for tool metrics endpoint analysis.
 * 
 * <p>
 * This class provides computed statistics and insights about tool metrics endpoint
 ...
 */
@NonNullByDefault
public record ToolMetricsEndpointStatistics(List<AgentTaskSnapshot> snapshots, Duration timeRange, long timestampMs)
        implements
            StatisticsSnapshot,
            CountsMetrics,
            LatencyMetrics,
    ...
    // EndpointMetrics implementation
    @Override
    public double responseTime() {
        if (snapshots.isEmpty())
            return 0.0;
        return snapshots.stream().mapToDouble(s -> s.averageMs()).average().orElse(0.0);
    }
    ...
    public double concurrency() {
        if (snapshots.isEmpty())
            return 0.0;
        return snapshots.stream().mapToDouble(s -> s.operationsPerSecond()).average().orElse(0.0);
    }
     ...
     * @return tool metrics endpoint statistics
     */
    public static ToolMetricsEndpointStatistics fromSnapshots(List<AgentTaskSnapshot> snapshots, Duration timeRange) {
        return new ToolMetricsEndpointStatistics(snapshots, timeRange, System.currentTimeMillis());
    }
     ...
     * 
     * @param timeRange the time range for statistics
     * @return empty tool metrics endpoint statistics
     */
    public static ToolMetricsEndpointStatistics empty(Duration timeRange) {
        return new ToolMetricsEndpointStatistics(List.of(), timeRange, System.currentTimeMillis());
    }
}
