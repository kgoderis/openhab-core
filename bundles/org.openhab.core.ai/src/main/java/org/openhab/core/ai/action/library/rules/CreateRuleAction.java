...
public class CreateRuleAction implements Action {
    ...
    public ActionValidationResult validateParameters(Map<String, Object> parameters) {
        try {
            ...
            return ActionValidationResult.invalid(List.of("Parameter validation error: " + e.getMessage()));
        }
    }
    ...
    public ActionResult execute(Map<String, Object> parameters, ExecutionContext context) throws ActionException {
        try {
            ...
            throw new ActionException(getActionId(), "Failed to create rule: " + e.getMessage(), e);
        }
    }
    ...
    private void validateRuleParameters(String ruleUID, String name, List<Map<String, Object>> triggers,
            List<Map<String, Object>> actions) throws ActionException {
        if (ruleUID == null || ruleUID.trim().isEmpty()) {
            throw new ActionException(getActionId(), "Rule UID is required");
        }

        if (name == null || name.trim().isEmpty()) {
            throw new ActionException(getActionId(), "Rule name is required");
        }

        // Check if rule already exists
        if (ruleRegistry.get(ruleUID) != null) {
            throw new ActionException(getActionId(), "Rule with UID '" + ruleUID + "' already exists");
        }

        // Validate triggers and actions
        if (triggers == null || triggers.isEmpty()) {
            throw new ActionException(getActionId(), "At least one trigger is required");
        }

        if (actions == null || actions.isEmpty()) {
            throw new ActionException(getActionId(), "At least one action is required");
        }
    }
