...
public class ProviderMetrics {
    ...
    public void recordExecution(boolean success, long executionTime) {
        ...
        if (success) { successfulExecutions.incrementAndGet(); } else { totalFailures.incrementAndGet(); }
    }
    ...
    public double getAverageResponseTime() { return totalExecutions.get() > 0 ? (double) totalExecutionTime.get() / totalExecutions.get() : 0.0; }
    public double getHealthScore() { return getSuccessRate() * (1.0 - Math.min(getAverageResponseTime() / 10000.0, 1.0)); }
}
