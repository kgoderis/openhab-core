...
class AgentSecurityManagerTest {
    ...
    @BeforeEach
    void setUp() {
        securityManager = new AgentSecurityManager();
    }
    ...
    void testHasA2APermissionWithA2APermissionEnum() {
        ...
        boolean result = securityManager.hasA2APermission(mockAuthContext, AgentSecurityManager.A2APermission.EXECUTE);
    ...
    void testGetSecurityStatistics() {
        AgentSecurityManager.SecurityStatistics statistics = securityManager.getSecurityStatistics();
    ...
    void testA2APermissionEnum() {
        ...
        assertEquals("connect", AgentSecurityManager.A2APermission.CONNECT.getPermission());
        assertEquals("execute", AgentSecurityManager.A2APermission.EXECUTE.getPermission());
        assertEquals("read", AgentSecurityManager.A2APermission.READ.getPermission());
        assertEquals("write", AgentSecurityManager.A2APermission.WRITE.getPermission());
        assertEquals("admin", AgentSecurityManager.A2APermission.ADMIN.getPermission());
    }

    @Test
    void testSecurityStatisticsConstructor() {
        AgentSecurityManager.SecurityStatistics statistics = new AgentSecurityManager.SecurityStatistics(5, 10, 2, true,
                false, 50, 30, 3);
    ...
    void testSecurityManagerLifecycle() {
        ...
        AgentSecurityManager.SecurityStatistics stats = securityManager.getSecurityStatistics();
        assertNotNull(stats);
