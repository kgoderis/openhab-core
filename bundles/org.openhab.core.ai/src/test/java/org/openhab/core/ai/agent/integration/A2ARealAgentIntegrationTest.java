...
class A2ARealAgentIntegrationTest {
    ...
    private boolean checkRealAgentAvailability() {
        try {
            HttpRequest request = HttpRequest.newBuilder().uri(URI.create(REAL_AGENT_URL + AGENT_CARD_ENDPOINT))
    ...
    void testRealAgentSkillExecution() throws Exception {
        ...
        if (executionResponse.has("result")) {
            ...
            System.out
                    .println("Real agent skill execution failed: " + skillName + " - " + error.get("message").asText());
        }
    }
    ...
    void testRealAgentProtocolCompliance() throws Exception {
        ...
        assertEquals(-32600, error.get("code").asInt(),
                "Real agent should return invalid request error for wrong JSON-RPC version");
