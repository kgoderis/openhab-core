...
- **Common Bundle**: Contains all `Action` implementations (e.g., `SystemInfoAction`, `ConfigurationGetAction`)
- **MCP Bundle**: Contains adapters that convert `Action` instances into MCP tools (e.g., `MCPToolAdapter`)
- **A2A Bundle**: Contains adapters that convert `Action` instances into A2A skills (e.g., `A2ASkillAdapter`)
...
- [ ] **Unit Tests**
  ...
  - [ ] Test `AgentSkillRegistry` and `A2ASkillAdapter`
    - [ ] Test skill registration and management
    - [ ] Test Action to AgentSkill conversion
...
- [ ] Add streaming support to the Actions
...
- [x] **A2A Persistence - Simplified Implementations** 
  ...
  - [x] Replace simplified approach in AgentOpenHABPersistenceManager with real implementation
