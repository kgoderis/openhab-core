...
- [x] **Update DefaultConfigurationService** with improved validation and error handling
- [x] **Update DefaultModelConfigurationService** with enhanced provider validation and `ai.model.*` key support
- [x] **Update AgentConfigurationManager** with better configuration parsing and `ai.agent.*` key support
- [x] **Create ToolConfigurationService** following openHAB patterns with `ai.tool.*` key support

#### 1.2 Implement Configuration Validation
- [x] **Create ConfigurationValidator** for manual validation of configuration maps
- [x] **Create ModelConfigurationValidator** for provider-specific validation with `ai.model.*` keys
- [x] **Create AgentConfigurationValidator** for agent-specific validation with `ai.agent.*` keys
- [x] **Create ToolConfigurationValidator** for tool-specific validation with `ai.tool.*` keys
- [x] **Add validation to all @Activate and @Modified methods**
- [x] **Implement configuration error recovery and logging**

### Phase 2: YAML Configuration Infrastructure (Week 2-3)

#### 2.1 Create YAML Configuration Services
- [x] **Create PromptRepository** for YAML prompt templates
- [x] **Create PolicyRepository** for YAML policy definitions
- [x] **Create ModelPresetRepository** for YAML model configurations
- [x] **Create AgentConfigurationRepository** for YAML agent configs

#### 2.2 Implement File Monitoring
- [x] **Extend WatchService integration** for YAML files
- [x] **Add YAML file change detection** and reload
- [x] **Implement YAML validation** with error reporting
- [x] **Add YAML schema validation** for complex structures

#### 2.3 Create YAML Parsers
- [x] **Create PromptYamlParser** with template support
- [x] **Create PolicyYamlParser** with rule validation
- [x] **Create ModelPresetYamlParser** with provider mapping
- [x] **Create AgentConfigYamlParser** with dependency resolution

### Phase 3: Configuration Integration (Week 3-4)

#### 3.1 Unified Configuration Access
- [x] **Create ConfigurationManager** as unified access point
- [x] **Implement configuration precedence** (env > cfg > yaml > defaults)
...
#### 4.1 Package Consolidation
- [x] **Move configuration value objects** from `/common/configuration` to `/config/model`
- [x] **Deprecate old ConfigurationChangeListener** interface
- [x] **Update all import statements** to use new package structure
- [x] **Fix ConfigurationChangeEvent** method calls throughout codebase
- [x] **Update test files** to use new package structure
- [x] **Verify compilation** and test execution

#### 4.2 Configuration System Unification
- [x] **Consolidate configuration management** under `/config` package
- [x] **Maintain backward compatibility** with deprecated interfaces
- [x] **Ensure all configuration classes** follow unified patterns
- [x] **Validate configuration system** integration

#### 4.3 Cleanup and Finalization
- [x] **Remove deprecated ConfigurationChangeListener** interface
- [x] **Remove old configuration package** completely
- [x] **Verify no remaining references** to old package
- [x] **Confirm all tests pass** after cleanup

### Phase 5: Domain-Based Configuration Organization (Completed)

#### 5.1 Domain Analysis and Reorganization
- [x] **Analyze current /config/model package structure** - identified domain-specific configuration classes
- [x] **Propose domain-based organization** - move configuration classes to their respective domains
- [x] **Create domain configuration packages**:
  - [x] `/action/configuration` for ActionExecutionConfiguration
  - [x] `/agent/configuration` for AgentModelConfiguration  
  - [x] `/security/configuration` for SecurityConfiguration
  - [x] `/reasoning/configuration` for MultiStepReasoningConfiguration
  - [x] `/tool/configuration` for ToolServerConfiguration
  - [x] `/config/common` for cross-domain BaseConfiguration, Configuration interface, and ServerConfiguration
- [x] **Move configuration classes** to appropriate domain packages
- [x] **Update package declarations** in all moved files
- [x] **Update import statements** throughout the codebase
- [x] **Update test files** to use new package structure
- [x] **Verify compilation** and test execution
- [x] **Ensure domain alignment** - each configuration class now lives in its respective domain

### Phase 6: Testing and Validation (Week 4-5)

#### 6.1 Configuration Testing
- [x] **Create unit tests** for all configuration services
- [x] **Create integration tests** for configuration loading
- [x] **Create YAML validation tests**
- [x] **Create configuration precedence tests**

#### 6.2 Error Handling Testing
- [x] **Test configuration error recovery**
- [x] **Test YAML parsing error handling**
- [x] **Test configuration validation errors**
- [x] **Test configuration change events**
