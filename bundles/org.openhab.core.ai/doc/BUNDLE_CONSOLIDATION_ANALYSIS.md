...
- **Key Components**:
  - `Action` interface and related classes
  - `ActionRegistry` for action discovery
...
- **ActionRegistry**: Used by both MCP and A2A for action discovery
- **Action interface**: Core abstraction used by both protocols
...
- `ActionRegistry` - Action discovery
- `Action` interface - Core action abstraction
- `ActionContext`, `ActionResult` - Action execution
...
#### ✅ Consistent Patterns
- **AI* prefix**: `Action`, `ActionRegistry`, `ActionContext`, `ActionResult`
